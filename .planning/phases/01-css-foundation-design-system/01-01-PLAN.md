...
3. Create `dashboard/src/styles/glassmorphism.css`:
    ...
    * Requires translucent background — backdrop-filter is invisible on opaque elements. */
   ...
   /* Neon Glow States (DS-03)
    * Box-shadow glow on hover/focus for interactive elements.
    * Apply .neon-glow to buttons, links, inputs. */
...
- Scanline overlay MUST have `pointer-events: none`
  ...
  <done>
    ...
    - `glassmorphism.css` provides .glass, .glass-strong, .glass-hover utility classes AND .neon-glow, .neon-glow-orange hover/focus glow states
